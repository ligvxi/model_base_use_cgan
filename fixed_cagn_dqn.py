...
from DQN import DQN
from CGANModel.CGAN import CGAN
from CGANModel.utils import load_pong_dqn
...
def startTraining(gan):
    ...
    while (lastResult < 14):
        ...
        for i in range(58):
            ...
            f, _, _, _ = env.step(sample_action)
        ...
        while not (d):
            ...
            newState = gan.get_next_s(state, a)
            new_r = DQN.get_reward(newState)
            if new_r == old_r:
                real_r = 0
            else:
...
if __name__ == '__main__':
    with tf.Session() as sess:
        gan = CGAN(sess,
                   ...
                   batch_size=128,
                   z_dim=84,
                   dataset_name="./CGANModel/data/pong",
        ...
        try:
            ...
            memory = replayMemory(sizeMemory=repBufferSize, sampleSize=sampleSize, image_height=height,
                                  image_width=width, num_frames=4, fixed=True)
