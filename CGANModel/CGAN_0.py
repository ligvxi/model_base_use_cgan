...
class CGAN(object):
    ...
    def __init__(self, sess, epoch, batch_size, z_dim, dataset_name, checkpoint_dir, result_dir, log_dir):
        ...
        self.model_dir = "./" + self.checkpoint_dir + "/cgan"
        # self.model_dir = "./CGANModel/"+self.checkpoint_dir+"/cgan"
        ...
        self.obs_, self.obs, self.action = load_pong(self.dataset_name + "/pong")  # s_, s, a
        self.test_obs_, self.test_obs, self.test_action = load_pong(self.dataset_name + "/pong_test")  # s_, s, a
        print('train_shape:', len(self.obs_))
        print('test_shape:', len(self.test_obs))
    ...
    def generator(self, s, a, is_training=True, reuse=False):
        # Network Architecture is exactly same as in infoGAN (https://arxiv.org/abs/1606.03657)
    ...
    def build_model(self):
        ...
        G = self.generator(self.s, self.a, is_training=True, reuse=False)
        ...
        """" Testing """
        # for test
        self.fake_images = self.generator(self.s, self.a, is_training=False, reuse=True)
    ...
    def get_mse(self):
        samples = self.sess.run(self.fake_images,
                                feed_dict={self.s: self.test_obs, self.a: self.test_action})
    ...
    def visualize_results(self, epoch, size=5):
        ...
        samples = self.sess.run(self.fake_images,
                                feed_dict={self.s: s_list, self.a: a_list})  # (5, 84, 84, 4)
        ...
        for i in range(size):
            for j in range(4):
                ...
                plt.subplot(2, 4, j + 5)
                plt.imshow(obs[i][j])
            plt.savefig(self.result_dir + "/epoch{}_{}.png".format(str(epoch), str(i)))
