...
name = "only_history_data_DQN_" + game
...
def startTraining():
    ...
    print("\n\n STARTING TRAINING.. \n\n")
    DATA_DIR = "CGANModel/budget/pong"
    for episode, file in enumerate(os.listdir(DATA_DIR)):  # for episode in episodes:
        episode_reward = 0
        episode_qvalues = []

        state_list = np.load(os.path.join(DATA_DIR, file))['obs']
        a_list = np.load(os.path.join(DATA_DIR, file))['action']
        newState_list = np.load(os.path.join(DATA_DIR, file))['obs_']
        real_r_list = np.load(os.path.join(DATA_DIR, file))['r']
        d_list = np.load(os.path.join(DATA_DIR, file))['d']

        step = state_list.shape[0]
        for i in range(step):  # while not done
            state, a, real_r, d, newState = state_list[i], a_list[i], real_r_list[i], d_list[i], newState_list[i]
            # use cgan to generate s'
        ...
        # init real env for test and for draw reward curve
        env.reset()
        for i in range(60):
            f, _, _, _ = env.step(0)
